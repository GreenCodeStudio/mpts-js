...
export class ExpressionParser extends AbstractParser {
  constructor(text, fileName = null, filePositionOffset = null, fileLineOffset = null, fileColumnOffset = null) {
    super();
    this.text = text;
    this.position = 0;
  ...
  parseNormal(endLevel = 0) {
    ...
    while (this.position < this.text.length) {
      ...
      if (/\s/.test(char)) {
        ...
        let name = this.readUntil(/['"\(\)=\.\s:>/+\-*?,]/);
        if (name == 'true') lastNode = new TEBoolean(true);else if (name == 'false') lastNode = new TEBoolean(false);else lastNode = new TEVariable(name, partCodePosition);
      }
    }
    return lastNode;
  }
